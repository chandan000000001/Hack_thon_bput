...
Ingestion → Heuristic Engine → Risk Scoring → XAI Gateway (Groq → OpenRouter → rule-based, circuit-broken) → Alert Generation → Dashboard
               └─ heavy media/bulk analysis runs synchronously on the request thread (threadpool for blocking IO)
```
...
- **Phase C-1 — background workers (removed 2026-09-11):** deepfake media and bulk log analysis (`POST /events/bulk`) now always run synchronously (200) on the request thread; the Arq/Redis queue, `job_queue.py` and `app/workers/` were deleted. The `redis` compose service and `arq`/`redis` packages remain as unused infrastructure.
