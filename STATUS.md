...
| Phase C-1 | Background workers (Arq + Redis, 202 queued flow) — **REMOVED 2026-09-11**: analysis now always runs synchronously (200); the `redis` compose service and `arq`/`redis` packages remain as unused infrastructure | REMOVED | plan.md §43 (historical); removal touches `app/api/routes_analysis.py`, `app/api/routes_events.py`, `app/main.py`, `app/core/config.py` |
...
| Phase D-1 | API hardening: threadpool rule, row locking, cross-tenant 404s, single-read uploads (rate limiting **REMOVED 2026-09-11** — `app/core/rate_limit.py` deleted, no `429` middleware) | DONE | backend/app/core/asyncbridge.py, routes_incidents.py (with_for_update); evidence/reports/regression_api.md |
