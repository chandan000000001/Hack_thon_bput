...
export default function Login() {
  ...
  return (
    <div className="cyber-grid flex min-h-screen items-center justify-center bg-zinc-950 px-4">
      <div className="pointer-events-none fixed left-1/2 top-1/2 h-96 w-96 -tranzinc-x-1/2 -tranzinc-y-1/2 rounded-full bg-red-500/5 blur-3xl" />
      <div className="w-full max-w-md">
        ...
        <div className="rounded-2xl border border-zinc-700/50 bg-zinc-900/80 p-8 shadow-2xl backdrop-blur">
          {isMockMode() && (
            <div className="mb-5 text-center">
              <span className="rounded-md bg-red-500/15 px-2.5 py-1 font-mono text-[11px] font-bold tracking-wider text-red-400 ring-1 ring-red-500/50">
                MOCK MODE
              </span>
            </div>
          )}
          ...
          {/* Mode tabs */}
          <div className="mb-5 grid grid-cols-3 gap-1 rounded-lg bg-zinc-800/60 p-1">
            {(
              [
                ['signin', 'Sign In'],
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {mode === 'signup' && (
              <div>
                <label className="mb-1.5 block text-xs font-medium uppercase tracking-wider text-zinc-400">Full Name</label>
                <div className="relative">
                  <User className="absolute left-3 top-1/2 h-4 w-4 -tranzinc-y-1/2 text-zinc-500" />
                  <input
                    type="text"
                    required
                    value={fullName}
                    onChange={(e) => setFullName(e.target.value)}
                    className="w-full rounded-lg border border-zinc-700/60 bg-zinc-800/60 py-2.5 pl-10 pr-3 text-sm text-zinc-100 placeholder-zinc-500 outline-none focus:border-red-500/60"
            ...
            <div>
              ...
              <div className="relative">
                <Mail className="absolute left-3 top-1/2 h-4 w-4 -tranzinc-y-1/2 text-zinc-500" />
                <input
                  type="email"
                  required
                  value={email}
            ...
            {mode !== 'forgot' && (
              <div>
                ...
                <div className="relative">
                  <Lock className="absolute left-3 top-1/2 h-4 w-4 -tranzinc-y-1/2 text-zinc-500" />
                  <input
                    type="password"
                    required
                    value={password}
            ...
            {mode === 'signup' && (
              <div>
                <label className="mb-1.5 block text-xs font-medium uppercase tracking-wider text-zinc-400">Confirm Password</label>
                <div className="relative">
                  <KeyRound className="absolute left-3 top-1/2 h-4 w-4 -tranzinc-y-1/2 text-zinc-500" />
                  <input
                    type="password"
                    required
            ...
            {notice && (
              <div className="rounded-lg border border-zinc-300/40 bg-zinc-300/10 px-3.5 py-2.5 text-xs text-zinc-200">
                {notice}
              </div>
            )}
          ...
          <div className="mt-5 rounded-lg border border-zinc-700/50 bg-zinc-800/40 px-3.5 py-2.5 text-center">
            <p className="font-mono text-[11px] leading-relaxed text-zinc-400">
              Seeded demo accounts: <span className="text-red-400">admin@</span>,{' '}
              <span className="text-red-400">analyst@</span>,{' '}
              <span className="text-red-400">viewer@cyberguard.local</span>
            </p>
          </div>
        </div>
