...
export default function Dashboard() {
  ...
  const recentColumns: Column<Alert>[] = [
    { key: 'id', header: 'ID', render: (a) => <span className="font-mono text-xs text-cyan-400">{a.id}</span>, sortValue: (a) => a.id },
    { key: 'title', header: 'Title', render: (a) => <span className="line-clamp-1 max-w-xs text-slate-200">{a.title}</span>, sortValue: (a) => a.title },
    { key: 'module', header: 'Module', render: (a) => <span className="text-xs text-slate-400">{MODULE_LABELS[a.module]}</span>, sortValue: (a) => a.module },
    { key: 'severity', header: 'Severity', render: (a) => <SeverityBadge severity={a.severity} />, sortValue: (a) => a.severity },
    { key: 'risk', header: 'Risk', render: (a) => <span className="font-mono font-semibold text-slate-100">{a.riskScore}</span>, sortValue: (a) => a.riskScore },
    ...
    <div className="space-y-5">
      ...
      {loading || !data ? (
        ...
        <>
          {/* Stat cards */}
          <div className="grid grid-cols-2 gap-4 xl:grid-cols-6">
            <StatCard title="Events Analyzed" value={data.totalEventsAnalyzed} icon={LayoutDashboard} trend="up" trendValue="+12%" color="#06b6d4" />
            <StatCard title="Threats Detected" value={data.threatsDetected} icon={ShieldAlert} trend="up" trendValue="+8%" color="#ef4444" />
            <StatCard title="Phishing Attempts" value={data.phishingAttempts} icon={Mail} trend="up" trendValue="+15%" color="#f97316" />
            <StatCard title="Impersonation Attempts" value={data.impersonationAttempts} icon={UserX} trend="neutral" trendValue="stable" color="#f59e0b" />
            <StatCard title="Suspected Deepfakes" value={data.suspectedDeepfakes} icon={Video} trend="up" trendValue="+5%" color="#a855f7" />
            <StatCard title="Account Takeover Attempts" value={data.accountTakeoverAttempts} icon={KeyRound} trend="down" trendValue="-3%" color="#f43f5e" />
          </div>
          ...
          <div className="grid gap-4 lg:grid-cols-3">
            <div className="space-y-4 lg:col-span-2">
              <ChartCard title="Risk Distribution" subtitle="Alerts by severity band">
                <ResponsiveContainer width="100%" height={220}>
                  <PieChart>
                    <Pie data={data.riskDistribution} dataKey="value" nameKey="name" innerRadius={60} outerRadius={85} paddingAngle={3} stroke="#0f172a">
              ...
              <ChartCard title="Attack Timeline — Last 24 Hours" subtitle="Events analyzed vs threats detected">
                <ResponsiveContainer width="100%" height={220}>
                  <AreaChart data={data.attackTimeline} margin={{ top: 4, right: 8, left: -18, bottom: 0 }}>
                    ...
                    <Tooltip contentStyle={tooltipStyle} />
                    <Area type="monotone" dataKey="events" stroke="#06b6d4" fill="url(#gEvents)" strokeWidth={2} name="Events" />
            ...
            <div className="space-y-4">
              <div className="rounded-xl border border-slate-700/50 bg-slate-800/60 p-4 backdrop-blur">
                <h3 className="mb-3 text-sm font-semibold text-slate-100">Incident Summary</h3>
                <div className="space-y-2.5">
                  {(
                    ...
                    <div key={status} className="flex items-center justify-between">
                      ...
                      <span className="font-mono text-lg font-bold text-slate-100">{count}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="rounded-xl border border-slate-700/50 bg-slate-800/60 p-4 backdrop-blur">
                <h3 className="mb-3 text-sm font-semibold text-slate-100">Top Targeted Users</h3>
                <div className="space-y-2">
                  {data.topTargetedUsers.map((u) => (
                    <div key={u.user} className="flex items-center justify-between gap-2">
                      <span className="truncate font-mono text-xs text-slate-300">{u.user}</span>
                      <span className="shrink-0 rounded bg-red-500/15 px-1.5 py-0.5 font-mono text-[11px] font-bold text-red-400">{u.attacks}</span>
                    </div>
                  ))}
                  {data.topTargetedUsers.length === 0 && <p className="text-xs text-slate-500">No targeted users recorded</p>}
                </div>
              </div>

              <div className="rounded-xl border border-slate-700/50 bg-slate-800/60 p-4 backdrop-blur">
                <h3 className="mb-3 text-sm font-semibold text-slate-100">Top Targeted Services</h3>
                <div className="space-y-2.5">
                  {data.topTargetedServices.map((s) => (
                    <div key={s.service} className="flex items-center justify-between gap-2">
                      <span className="truncate text-xs text-slate-300">{s.service}</span>
                      <div className="flex shrink-0 items-center gap-1.5">
                        <span className="font-mono text-[11px] text-slate-500">{s.attacks}x</span>
                        <SeverityBadge severity={s.riskLevel} />
                      </div>
                    </div>
                  ))}
                  {data.topTargetedServices.length === 0 && <p className="text-xs text-slate-500">No targeted services recorded</p>}
                </div>
              </div>
            </div>
          </div>
          ...
          {/* Recent alerts */}
          <div>
            <h3 className="mb-2 text-sm font-semibold text-slate-200">Recent Alerts</h3>
            <DataTable
